...
- **FinalBurn Neo** — same treatment: point at the standalone `fbneo64.exe` and a ROM folder; titles come from a cached `fbneo -listinfo` dump, art matches by shortname, and games show under their own "FBNeo" category. Two FBNeo-specific notes: use FBNeo-compatible romsets (they differ from MAME sets), and set the same ROM folder inside FBNeo's own settings, since FBNeo locates ROMs through its internal paths at launch. BIOS sets (neogeo.zip etc.) are skipped from the wheel automatically. To refresh the title cache, delete `fbneo_titles.json` next to the config file.
- **RetroArch** — first-class: configure the executable once (**Settings → RetroArch**; the cores folder is auto-detected), then **+ Add RetroArch system** and pick from presets — NES, SNES, Genesis, GB/GBC, GBA, N64, PlayStation, PC Engine, Master System, Atari 2600, Dreamcast — with the core and file extensions prefilled, so only the ROM folder is left to enter. Each system appears as its own category. Cores must be installed inside RetroArch itself (Online Updater → Core Downloader); the core name in a preset (e.g. `snes9x`) maps to `snes9x_libretro.dll`/`.so` in the cores folder.
- **App shortcuts** — any standalone program as a wheel entry (**Settings → + Add app shortcut**): name, executable, optional args and category. Launch and return-to-wheel work like everything else. Right-click the tile to give it an icon image.

  **Fightcade**: add the client (`Fightcade2.exe` on Windows, `Fightcade2.sh` on Linux) as an app shortcut — that's your entry into online lobbies (a mouse/trackball helps navigate its UI). For *offline* play of the same games, point the **FinalBurn Neo** section at Fightcade's bundled emulator (`Fightcade\emulator\fbneo\fcadefbneo.exe`) and its ROM folder (`Fightcade\emulator\fbneo\ROMs`) — it's an FBNeo fork with the same CLI, so titles, art, and per-game launching all work through the existing FBNeo support.
